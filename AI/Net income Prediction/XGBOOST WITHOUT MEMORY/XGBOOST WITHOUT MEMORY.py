...
def predict_net_income(df):
    ...
    actual_direction = y_test.values - y_train.mean()  # Use mean as reference for year-over-year comparison
    predicted_direction = y_pred - y_train.mean()

    direction_match = np.sign(actual_direction) == np.sign(predicted_direction)
    directional_accuracy = np.mean(direction_match) * 100

    print(f'Directional Accuracy: {directional_accuracy:.2f}%')
    within_5_percent = np.abs((y_pred - y_test) / y_test) <= 4
    accuracy_within_5_percent = np.mean(within_5_percent) * 100
    print(f'Accuracy within 400%: {accuracy_within_5_percent:.2f}%')
...
def plot_percentage_gap(df_predicted):
    ...
    plt.show()
# Usage
file_path = '../Net income Prediction/financial_comparisons.csv'  # Your file with 2022 financial data
excel_file = '../Net income Prediction/combined_financial_data_all_stocks.xlsx'  # Your file with 2023 data
output_file = '../Net income Prediction/financial_predictions_with_regression.csv'  # Output file
