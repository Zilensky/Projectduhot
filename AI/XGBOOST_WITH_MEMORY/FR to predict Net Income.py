...
model=None
def predict_net_income(df, tune_model=True, lag_features=1, model=None):
    lag_features = 1
    ...
    return df
def predict_for_a_year(df,df_excel,pre,post):
    ...
    # Predict and save
    return predict_net_income(df_combined,True,1,model)
def combine_and_predict(file_path, excel_file, output_file):
    ...
    # Merge datasets
    df_2022 = df[df['Year'] ==2020].drop(['Year'], axis=1)
    df_2023 = df_excel[df_excel['Year'] == 2021][['Symbol', 'Total Revenue']]
    df_combined = pd.merge(df_2022, df_2023, on='Symbol', how='left', suffixes=('', '_2022'))
    ...
    # Predict and save
    df_predicted = predict_net_income(df_combined,True,1,model)

 #   predict_for_a_year(df,df_excel,2019,2020)
  #  predict_for_a_year(df,df_excel,2021,2022)
